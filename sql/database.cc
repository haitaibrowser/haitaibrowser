...
bool IsDbEncrypted(const char* szdbname)
{
  sqlite3* db = 0;
  sqlite3_stmt* stmt = 0;
  char szsql[512] = { 0 };
  ...
  if (SQLITE_OK != sqlite3_open(szdbname, &db))
  {
    goto err;
  }
  sprintf(szsql, "SELECT * FROM sqlite_master;");
  if (SQLITE_OK != (nret = sqlite3_prepare(db, szsql, -1, &stmt, NULL)))
  {
    nret = 1;
    goto err;
  }
...
err:
  if (nret != 0) {
    err = sqlite3_key(db, "HTDF123456", 10);
    err = sqlite3_rekey(db, nullptr, 0);
  }
  if (stmt)
...
// add by jie.zhang 20210130 for enc db
bool Database::DBKey(const base::FilePath& path, bool bOpen) {
  int err = 0;
  if (bOpen) {
    IsDbEncrypted(AsUTF8ForSQL(path).c_str());
  }
  else {
    err = sqlite3_rekey(db_, "HTDF123456", 10);
  }
  return true;
}
// end add by jie.zhang 20210130 for enc db
...
bool Database::OpenInternal(const std::string& file_name,
  ...
  //bool bIsDbEncrypted = IsDbEncrypted(file_name.c_str());
  ////int err = sqlite3_open(file_name.c_str(), &db_);
  //if (bIsDbEncrypted) {
  // // sqlite3_key(db_, "HTDF123456", 10);
  //}
  //else {
  // // err = sqlite3_rekey(db_, "HTDF123456", 10);
  //}
